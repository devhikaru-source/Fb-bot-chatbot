        ...
        var listenEmitter = api.listenMqtt(async (error, event) => {
          ...
          if (event.body && aliases(command)?.name) {
            ...
            if (isDevOnly) {
              if (!dev.includes(event.senderID)) {
                return api.sendMessage("🚫 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱.\n\nYou dont have access to this command, you need to be a developer.", event.threadID, event.messageID)
              }
            }
          ...
          if (event.body && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && aliases(command)?.name) {
            if (blacklist.includes(event.senderID)) {
              api.sendMessage("⚠️ 𝘄𝗮𝗿𝗻𝗶𝗻𝗴.\n\nWe're sorry, but you've been banned from using bot. If you believe this is a mistake or would like to appeal, please contact one of the bot admins for further assistance.", event.threadID, event.messageID);
              return;
            }
          }
          if (event.body && aliases(command)?.name) {
